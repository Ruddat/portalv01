...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('mod_orders', function (Blueprint $table) {
            ...
            $table->integer('status')->default(0)->comment('0 - new (created), 1 - ready to send, 2 - restaurant received, 3 - baking, 4 - delivering, 5 - boxing (packing), 6 - delivered (done), 400 - error, 500 - canceled');
            ...
            $table->decimal('price_products', 10, 0);
            $table->integer('price_bottles')->default(0);
            $table->decimal('price_shipping', 10, 0);
            $table->decimal('price_payment', 10, 0);
            $table->decimal('price_tips', 10, 0);
            $table->decimal('coupon_discount', 10, 0)->default(0);
