...
<livewire:frontend.footershop.footer-shop-infos-component :shopId="$restaurant->id" />



    <footer>
    <div class="margin_60_40 fix_mobile container">
        ...
        <div class="row add_bottom_25">
                    ...
                    <li>
                        <div class="styled-select lang-selector">
                            <select class="change_lang">
                                <option value="de"
                                        {{ session('locale') == 'de' ? 'selected' : '' }}>Deutsch</option>
                                <option value="en"
                                        {{ session('locale') == 'en' ? 'selected' : '' }}>English</option>
                                <option value="nl"
                                        {{ session('locale') == 'nl' ? 'selected' : '' }}>Niederländisch
                                </option>
                                <option value="fr"
                                        {{ session('locale') == 'fr' ? 'selected' : '' }}>French</option>
                                <option value="es"
                                        {{ session('locale') == 'es' ? 'selected' : '' }}>Spanish
                                </option>
                                <option value="ru"
                                        {{ session('locale') == 'ru' ? 'selected' : '' }}>Russian
                                </option>
                                <option value="ar"
                                        {{ session('locale') == 'ar' ? 'selected' : '' }}>Arabisch
                                </option>
                                <option value="fa"
                                        {{ session('locale') == 'fa' ? 'selected' : '' }}>Persisch
                                </option>
                                <option value="pl"
                                        {{ session('locale') == 'pl' ? 'selected' : '' }}>Polnisch
                                </option>
                                <option value="tr"
                                        {{ session('locale') == 'tr' ? 'selected' : '' }}>Türkisch
                                </option>
                                <option value="uk"
                                        {{ session('locale') == 'uk' ? 'selected' : '' }}>Ukrainisch
                                </option>
                            </select>
                        </div>
                    </li>
            ...
            <div class="col-lg-6">
                <ul class="additional_links">
                    <li><a
                           href="/impressum">{{ app(\App\Services\AutoTranslationService::class)->trans('Impressum', app()->getLocale()) }}</a>
                    </li>
                    <li><a
                           href="#0">{{ app(\App\Services\AutoTranslationService::class)->trans('Terms and conditions', app()->getLocale()) }}</a>
                    </li>
                    <li><a
                           href="#0">{{ app(\App\Services\AutoTranslationService::class)->trans('Privacy', app()->getLocale()) }}</a>
                    </li>
            ...
            <div class="footer-copyright py-3">
                <div class="d-flex align-items-center container">
                    ...
                    <p class="text-muted d-flex align-items-center mb-0 ms-auto">


                        <a href="#"
                           class="d-block"><img alt="#"
                                 src="{{ asset('frontend/img/appstore.png') }}"
                                 height="40"></a>

                        <button id="install-button"
                                style="display: none; border: none; background: none;">
                            <img src="{{ asset('frontend/img/playmarket.png') }}"
                                 alt="Install Icon"
                                 style="height: 40px;">
                        </button>
                    </p>
        ...
        <div id="install-instructions"
             style="display: none;">
            <p>Um die App zu installieren, tippe auf <strong>Teilen</strong> und dann auf <strong>Zum Startbildschirm
                    hinzufügen</strong>.</p>
        </div>
